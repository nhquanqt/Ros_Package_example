...
Mat DetectLane::birdViewTranform(const Mat &src)
{
    ...
    /*    
    Mat debug;
    cvtColor(src,debug,cv::COLOR_GRAY2BGR);
    for(int i = 1; i < 4; ++i){
        line(debug,src_vertices[i-1],src_vertices[i],Scalar(0,0,255),2,LINE_8);
    }
    line(debug,src_vertices[0],src_vertices[3],Scalar(0,0,255),2,LINE_8);
    imshow("Debug",debug);
    */
    ...
    for(int i = 1; i < 4; ++i){
        line(debug,dst_vertices[i-1],dst_vertices[i],Scalar(0,0,255),2,LINE_8);
    }
    line(debug,dst_vertices[0],dst_vertices[3],Scalar(0,0,255),2,LINE_8);
    imshow("Debug",debug);
    */
